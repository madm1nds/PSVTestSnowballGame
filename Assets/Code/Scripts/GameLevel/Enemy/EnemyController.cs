...
public class EnemyController : MonoBehaviour
{
    ...
    private const float speedRotateEnemySnowball = 15f;
    private const float correctionSpeedEnemySnowball = 4f;
    private const float snowballUpdateTimeAccess = 3.8f;
    private const float timeReturningSnowballPosition = 0.5f;
            ...
            for (int i = 0; i < Vault.instance.gameObjectEnemySnowballSet.Length; i++)
            {
                    ...
                    (Vault.instance.settings.evasionMode == false && TimerCooldownNormalModeController.isTimeOut == true && !isMoveOut)))
                {
                    if (Vault.instance.gameObjectEnemySnowballSet[i].activeInHierarchy == false)
                    {
                        ...
                        Vault.instance.transformEnemyTargetSet[i].position = new Vector3(Vault.instance.transformTargetPlayer.position.x,
                            Vault.instance.transformTargetPlayer.position.y, Vault.instance.gameObjectEnemySnowballSet[i].transform.position.z);
    ...
    IEnumerator MoveSnowball(GameObject enemySnowball, Transform target)
    {
        ...
        do
        {
            ...
            {
                timer += 0.015f;
                enemySnowballTransform.position = Vector3.MoveTowards(enemySnowballTransform.position, target.position, characterSettings.speedSnowball / correctionSpeedEnemySnowball);
                enemySnowballTransform.Rotate(0, 0, speedRotateEnemySnowball);
                target.position = Vector3.MoveTowards(target.position, enemySnowballTransform.position, -characterSettings.speedSnowball / correctionSpeedEnemySnowball);
        ...
        } while (enemySnowball.GetComponent<CircleCollider2D>().enabled == true && timer <= snowballUpdateTimeAccess);
        ...
        yield return new WaitForSeconds(timeReturningSnowballPosition);
                        ...
                        if (currentEnemyTransform.position.y > newEnemyLocation)
                        {
                            while (currentEnemyTransform.position.y > newEnemyLocation)
                            {
                                if (PauseButtonController.isPause == false)
                                {
                                    ...
                                    currentEnemyTransform.position = new Vector3(currentEnemyTransform.position.x, currentEnemyTransform.position.y - currentSpeedEnemy,
                                            currentEnemyTransform.position.z);
